...
def _revise_prompt_with_gpt52(source_image_path: Path, desired_image_prompt: str, model: str = "gpt-5.2") -> str:
    ...
    user = (
        ...
        "目前正在更新的小区名字为“逸成东苑”"
        "请在理解现场图片的基础上，将上述内容改写为一段可直接用于【在原照片基础上进行编辑】的真实可落地图生图/图改图 prompt。"
        "改写时请遵循以下要求：\n"
        ...
        "- 如需求中涉及低空经济、智慧物流或相关应用场景，需要在 prompt 中补充对无人机和无人车的描述；其尺度、形象、数量、材质感、空间位置和运行/停放关系必须符合真实场景与透视关系，"
        "应自然融入小区环境，不得夸张、悬浮失真或出现明显科幻化表达，并避免与建筑、树木、道路、人行流线和安全边界发生冲突。\n"
        "- 在风格上应保持与原照片整体视角、构图和时间段（如白天/夜间）大体一致，只对内容和秩序进行优化，而非完全换一个场景。\n"
        "- 输出一段连续的英文描述，用于直接作为图改图模型的 prompt，不要添加任何解释性语句、前后缀或列表符号。"
    )
